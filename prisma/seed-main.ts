...
export async function seedMain() {
  ...
  // HASH PASSWORD
  // ------------------------
  const passwordSuper = await bcrypt.hash('superadmin_123', 10)
  const passwordAdmin = await bcrypt.hash('admin_123', 10)
  const passwordUser = await bcrypt.hash('user_123', 10)
  ...
  if (!roleUser || !roleAdmin || !roleSuperAdmin) {
    throw new Error('Roles missing')
  }

  // ------------------------
  // USERS (buat superadmin, admin, dan 5 user biasa untuk dosen)
  // ------------------------
  // Hapus semua user sebelumnya jika ada (opsional, karena reset database sudah hapus)
  // Tapi kita tetap gunakan createMany dengan skipDuplicates agar aman.
  const usersData = [
    {
      email: 'budi.santoso@umn.ac.id',
    ...
    {
      email: 'rina.wijaya@umn.ac.id',
      password: passwordUser,
      name: 'Rina Wijaya',
      roleId: roleUser.id,
    },
    {
      email: 'hendra.gunawan@umn.ac.id',
      password: passwordUser,
      name: 'Hendra Gunawan',
      roleId: roleUser.id,
    },
    {
      email: 'dewi.kartika@umn.ac.id',
      password: passwordUser,
      name: 'Dewi Kartika',
      roleId: roleUser.id,
    },
    {
      email: 'bambang.prasetyo@umn.ac.id',
      password: passwordUser,
      name: 'Bambang Prasetyo',
      roleId: roleUser.id,
    },
    {
      email: 'lina.susanti@umn.ac.id',
      password: passwordUser,
      name: 'Lina Susanti',
      roleId: roleUser.id,
    },
  ...
  // Ambil semua user dengan role 'user' (5 orang)
  const regularUsers = await prisma.user.findMany({
    where: { roleId: roleUser.id },
  })

  if (regularUsers.length < 5) {
    throw new Error('Tidak cukup user dengan role user untuk dijadikan dosen')
  }

  // ------------------------------------------------------------------------
  // LECTURERS (relasi ke user biasa)
  // ------------------------------------------------------------------------
  const lecturersData = [
    {
  ...
  // ------------------------------------------------------------------------
  // PUBLICATIONS (20 realistic titles)
  // ------------------------------------------------------------------------
  ...
  // ------------------------------------------------------------------------
  // PUBLICATION AUTHORS (each pub has 2-5 authors, at least one lecturer)
  // ------------------------------------------------------------------------
  ...
  // ------------------------------------------------------------------------
  // NEWS (10 entries, by admin/superadmin)
  // ------------------------------------------------------------------------
